...
export async function DELETE(req: NextRequest, context: ContextParams) {
  ...
  try {
    const form = await Form.findOne({ _id: params.id, owner: session.user.id });
    ...
    if (!form) {
      ...
      if (existingForm) {
        console.log("Form exists but user doesn't own it. Owner:", existingForm.owner, "User:", session.user.id);
        return NextResponse.json({ error: "Not authorized to delete this form" }, { status: 403 });
      }
      return NextResponse.json({ error: "Form not found" }, { status: 404 });
    }
    
    const deletedForm = await Form.findOneAndDelete({ _id: params.id, owner: session.user.id });
